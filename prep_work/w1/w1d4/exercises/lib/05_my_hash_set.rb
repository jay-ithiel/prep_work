...
class MyHashSet
  ...
  def union(set2)
    unions = self.class.new
    self.to_a.each { |key_value| unions.insert(key_value) }
    set2.to_a.each { |key_value| unions.insert(key_value) }
    unions
  end

  def intersect(set2)
    intersects = self.class.new
    self.to_a.each do |key_value|
      intersects.insert(key_value) if set2.to_a.include?(key_value)
    end
    intersects
  end

  def minus(set2)
    difference = self.class.new
    self.to_a.each do |key_value|
      difference.insert(key_value) if !set2.to_a.include?(key_value)
    end
    difference
  end
