...
class Course
  ...
  def initialize(name, department, credits, days = nil, time_block = nil)
    @name = name
    @department, @credits = department, credits
    @days, @time_block = days, time_block
    @students = []
  end
  ...
  def conflicts_with?(second_course)
    days_conflict?(second_course) && time_blocks_conflict?(second_course)
  end

  private
    def days_conflict?(second_course)
      same_days = self.days.select do |first_course_day|
        second_course.days.include?(first_course_day)
      end
      same_days.length == 0 ? false : true
    end

    def time_blocks_conflict?(second_course)
      self.time_block === second_course.time_block
    end
